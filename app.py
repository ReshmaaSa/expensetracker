...
def login():
    ...
    if request.method == 'POST' :
        ...
        account = cursor.fetchone()
        print ('aaaaaaaaaaaaaaaaa', account)
...
def addexpense():
    ...
    cursor = mysql.connection.cursor()
    print(" " + session["id"] + " " + date + " " + expensename + " " + amount + " " + paymode + " " + category)

    cursor.execute('INSERT INTO expenses VALUES (NULL,  % s, % s, % s, % s, % s, % s)', ('aaa' , str(date), expensename, amount, paymode, category,))
    mysql.connection.commit()
...
def display():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute('SELECT * FROM expenses WHERE userid = % s AND date ORDER BY `expenses`.`date` DESC',(str(session['username']), ))
    ...
    for expense in expenses:
        costs[expense[6]] += int(expense[4])
    return render_template('display.html' ,expense = expenses, total=sum(costs.values()), costs=costs)
...
def delete(id):
     cursor = mysql.connection.cursor()
     cursor.execute(f'DELETE FROM expenses WHERE  userid="{id}"')
     mysql.connection.commit()
...
def update(id):
  if request.method == 'POST' :
      ...
      # cursor.execute(f"UPDATE expenses SET date={date} , expensename={expensename} , amount={amount}, paymode={str(paymode)}, category={str(category)} WHERE expenses.userid={id}")
...
def limitnum():
     if request.method == "POST":
         ...
         cursor = mysql.connection.cursor()
         cursor.execute('INSERT INTO limits VALUES (NULL, % s, % s) ',(session['username'], number))
         mysql.connection.commit()
         return redirect('/limitn')
